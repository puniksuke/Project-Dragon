public final class ObjectDef {
	...
	public static void unpackConfig(StreamLoader streamLoader) {
		...
		// "loc.dat"));
		// Stream stream = new
		// Stream(FileOperations.ReadFile(signlink.findcachedir()+ "loc.idx"));
		...
		for (int i1 = 0; i1 < totalObjects; i1++) {
			if (forID(i1) == null)
				continue;
			if (forID(i1).name == null)
				continue;
			if (forID(i1).name.toLowerCase().contains("wild")) {
				System.out.println(forID(i1).name + " : " + i1);
			}
		}
	}

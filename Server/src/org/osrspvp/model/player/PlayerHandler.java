...
public class PlayerHandler {
	...
	public void process() {
		PathRequest.pollPendingRequests();
		updatePlayerNames();
			...
			try {
				...
				if (players[i].disconnected
					...
					if (((Client) players[i]).getTradeHandler().getStage() > 0) {
						((Client) players[i]).getTradeHandler()
								.cancelCurrentTrade();
					}
	...
	public void updatePlayer(Player plr, Stream str) {
		...
		for (int i = 0; i < Config.MAX_PLAYERS; i++) {
			...
			if (!plr.withinDistance(players[i]))
				continue;
			plr.addNewPlayer(players[i], str, updateBlock);// open player .java
			// for me
			j++;
		}
	...
	private void removePlayer(Player plr) {
		if (((Client) plr).getTradeHandler().stage > 0) {
		} else {
			PlayerSave.saveGame((Client) plr);
		}
		plr.destruct();
	}

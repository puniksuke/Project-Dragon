...
public class CombatAssistant {
    ...
    public boolean goodSlayer(int i) {
        for (int j = 0; j < slayerReqs.length; j++) {
            if (slayerReqs[j][0] == NPCHandler.npcs[i].npcType) {
                if (slayerReqs[j][1] > c.getLevel()[PlayerConstants.PLAYER_SLAYER]) {
    ...
    public void attackNpc(int i) {
        if (NPCHandler.npcs[i] != null) {
            ...
            if (NPCHandler.npcs[i].underAttackBy > 0 && NPCHandler.npcs[i].underAttackBy != c.playerId && !NPCHandler.npcs[i]
                .inMulti()) {
                c.npcIndex = 0;
            ...
            if (c.attackTimer <= 0) {
                ...
                boolean usingCross = c.getEquipment()[PlayerConstants.PLAYER_WEAPON] == 9185;
                c.bonusAttack = 0;
                c.rangeItemUsed = 0;
                c.projectileStage = 0;
                c.oldNpcIndex = -1;
                ...
                c.attackTimer = getAttackDelay(
                    c.getItems().getItemName(c.getEquipment()[PlayerConstants.PLAYER_WEAPON]).toLowerCase());
                c.specAccuracy = 1.0;
                c.specDamage = 1.0;
                if (!c.usingMagic) {
                ...
                if (armaNpc(i) && !usingCross && !c.usingBow && !c.usingMagic && !usingCrystalBow() && !c.usingRangeWeapon) {
                    resetPlayerAttack();
                    return;
                }
                if ((!c.goodDistance(c.getX(), c.getY(), NPCHandler.npcs[i].getX(), NPCHandler.npcs[i].getY(),
                    2) && (usingHally() && !c.usingRangeWeapon && !c.usingBow && !c.usingMagic)) || (!c
                    .goodDistance(c.getX(), c.getY(), NPCHandler.npcs[i].getX(), NPCHandler.npcs[i].getY(),
                        4) && (c.usingRangeWeapon && !c.usingBow && !c.usingMagic)) || (!c
                    .goodDistance(c.getX(), c.getY(), NPCHandler.npcs[i].getX(), NPCHandler.npcs[i].getY(),
                        1) && (!c.usingRangeWeapon && !usingHally() && !c.usingBow && !c.usingMagic)) || ((!c
                    .goodDistance(c.getX(), c.getY(), NPCHandler.npcs[i].getX(), NPCHandler.npcs[i].getY(),
                ...
                if (!usingCross && !usingArrows && c.usingBow && (c.getEquipment()[PlayerConstants.PLAYER_WEAPON] < 4212 || c
                ...
                if (!usingCorrectArrows() && Config.CORRECT_ARROWS && c.usingBow && !usingCrystalBow() && c
                    .getEquipment()[PlayerConstants.PLAYER_WEAPON] != 9185) {
                    c.sendMessage(
                        "You can't use " + c.getItems().getItemName(c.getEquipment()[PlayerConstants.PLAYER_ARROWS])
                            .toLowerCase() + "s with a " + c.getItems()
                            .getItemName(c.getEquipment()[PlayerConstants.PLAYER_WEAPON]).toLowerCase() + ".");
                    c.stopMovement();
                    c.npcIndex = 0;
                    return;
                }

                if (c.getEquipment()[PlayerConstants.PLAYER_WEAPON] == 9185 && !properBolts()) {
                ...
                if (c.usingBow || c.usingMagic || c.usingRangeWeapon || (c
                    .goodDistance(c.getX(), c.getY(), NPCHandler.npcs[i].getX(), NPCHandler.npcs[i].getY(),
                        2) && usingHally())) {
                    c.stopMovement();
                }
                ...
                if (c.usingSpecial && !c.usingMagic) {
                    if (checkSpecAmount(c.getEquipment()[PlayerConstants.PLAYER_WEAPON])) {
                        c.lastWeaponUsed = c.getEquipment()[PlayerConstants.PLAYER_WEAPON];
                        c.lastArrowUsed = c.getEquipment()[PlayerConstants.PLAYER_ARROWS];
                        activateSpecial(c.getEquipment()[PlayerConstants.PLAYER_WEAPON], i);
                        return;
                    } else {
                ...
                if (c.usingBow && !c.usingRangeWeapon && !c.usingMagic || usingCross) { // range
                    ...
                    c.projectileStage = 1;
                    c.oldNpcIndex = i;
                    if (c.getEquipment()[PlayerConstants.PLAYER_WEAPON] >= 4212 && c
                        .getEquipment()[PlayerConstants.PLAYER_WEAPON] <= 4223) {
                        c.rangeItemUsed = c.getEquipment()[PlayerConstants.PLAYER_WEAPON];
                        c.crystalBowArrowCount++;
                        c.lastArrowUsed = 0;
                    } else {
                        c.rangeItemUsed = c.getEquipment()[PlayerConstants.PLAYER_ARROWS];
                ...
                if (c.usingBow && Config.CRYSTAL_BOW_DEGRADES) { // crystal bow
                    // degrading
                    if (c.getEquipment()[PlayerConstants.PLAYER_WEAPON] == 4212) { // new
                        // crystal
                        // bow
                        // becomes
                        // full
                    ...
                    if (c.crystalBowArrowCount >= 250) {
                        switch (c.getEquipment()[PlayerConstants.PLAYER_WEAPON]) {
                        ...
                        default:
                            c.getItems().wearItem(++c.getEquipment()[PlayerConstants.PLAYER_WEAPON], 1, 3);
    ...
    public void delayedHit(int i) { // npc hit delay
        ...
        if (c.bowSpecShot >= 2) {
            c.bowSpecShot = 0;
            // c.attackTimer =
            // getAttackDelay(c.getItems().getItemName(c.getEquipment()[PlayerConstants.PLAYER_WEAPON]).toLowerCase());
        }
        if (c.bowSpecShot == 1) {
            fireProjectileNpc();
            c.hitDelay = 2;
        ...
        if (c.oldNpcIndex > 0) {
            if (NPCHandler.npcs[c.oldNpcIndex] != null) {
                ...
                if (c.lastWeaponUsed == 9185 || c.lastWeaponUsed == 11785) {
                    ...
                    if (c.armadylSpecial) {
                        c.getPA()
                            .createPlayersProjectile(pX, pY, offX, offY, 50, clientSpeed, 301, 46, 36, c.oldNpcIndex + 1,
                                showDelay, slope);
                    } else {
                        c.getPA()
                            .createPlayersProjectile(pX, pY, offX, offY, 50, clientSpeed, getRangeProjectileGFX(), 46, 36,
                    ...
                    slope = 15;
                    clientSpeed += 30;
                    c.getPA().createPlayersProjectile(pX, pY, offX, offY, 50, clientSpeed, getRangeProjectileGFX(), 41, 31,
                        c.oldNpcIndex + 1, 36, 3);
                    c.getPA()
                        .createPlayersProjectile(pX, pY, offX, offY, 50, clientSpeed + 10, getRangeProjectileGFX(), 46, 36,
                            c.oldNpcIndex + 1, 36, slope + 6);
                    return;
                } else if (c.msbSpec) {
                    c.getPA()
                        .createPlayersProjectile(pX, pY, offX, offY, 50, 40 + (distance * 5), 249, 43, 35, c.oldNpcIndex + 1,
                            36, 15);
                    c.getPA()
                        .createPlayersProjectile(pX, pY, offX, offY, 50, 65 + (distance * 5), 249, 43, 35, c.oldNpcIndex + 1,
                            36, 15);
                    return;
                } else {
    ...
    public void attackPlayer(int i) {
        ...
        if (PlayerHandler.players[i] != null) {
            ...
            if (PlayerHandler.players[i].isDead || PlayerHandler.players[i].getLevel()[3] <= 0 || c
                .getLevel()[3] <= 0 || c.isDead) {
                resetPlayerAttack();
                return;
            }
            ...
            if (c.attackTimer <= 0) {
                ...
                if (c.duelRule[9]) {
                    ...
                    for (int funWeapon : Config.FUN_WEAPONS) {
                        if (c.getEquipment()[PlayerConstants.PLAYER_WEAPON] == funWeapon) {
                            canUseWeapon = true;
                        }
                    }
                    if (!canUseWeapon) {
                ...
                if ((!c.goodDistance(c.getX(), c.getY(), PlayerHandler.players[i].getX(), PlayerHandler.players[i].getY(),
                    4) && (c.usingRangeWeapon && !c.usingBow && !c.usingMagic)) || (!c
                    .goodDistance(c.getX(), c.getY(), PlayerHandler.players[i].getX(), PlayerHandler.players[i].getY(),
                        2) && (!c.usingRangeWeapon && usingHally() && !c.usingBow && !c.usingMagic)) || (!c
                    .goodDistance(c.getX(), c.getY(), PlayerHandler.players[i].getX(), PlayerHandler.players[i].getY(),
                        getRequiredDistance()) && (!c.usingRangeWeapon && !usingHally() && !c.usingBow && !c.usingMagic)) || (!c
                    .goodDistance(c.getX(), c.getY(), PlayerHandler.players[i].getX(), PlayerHandler.players[i].getY(),
                        10) && (c.usingBow || c.usingMagic))) {
                ...
                if (!usingArrows && c.usingBow && (c.getEquipment()[PlayerConstants.PLAYER_WEAPON] < 4212 || c
                    .getEquipment()[PlayerConstants.PLAYER_WEAPON] > 4223) && !c.usingMagic) {
                    c.sendMessage("You have run out of arrows!");
                    c.stopMovement();
                    resetPlayerAttack();
                    return;
                }
                if (!usingCorrectArrows() && Config.CORRECT_ARROWS && c.usingBow && !usingCrystalBow() && !c.usingMagic) {
                    c.sendMessage(
                        "You can't use " + c.getItems().getItemName(c.getEquipment()[PlayerConstants.PLAYER_ARROWS])
                            .toLowerCase() + "s with a " + c.getItems()
                            .getItemName(c.getEquipment()[PlayerConstants.PLAYER_WEAPON]).toLowerCase() + ".");
                ...
                if (!c.attackedPlayers.contains(c.playerIndex) && !PlayerHandler.players[c.playerIndex].attackedPlayers
                    .contains(c.playerId)) {
    ...
    public boolean isAttackable(int i) {
        if (System
            .currentTimeMillis() - PlayerHandler.players[c.playerIndex].attackableTimer > 10000 && PlayerHandler.players[c.playerIndex]
            .inSafeZone()) {
            return true;
        }
        return false;
    ...
    public boolean usingCrystalBow() {
        return c.getEquipment()[PlayerConstants.PLAYER_WEAPON] >= 4212 && c
            .getEquipment()[PlayerConstants.PLAYER_WEAPON] <= 4223;
    }
    ...
    public void playerDelayedHit(int i) {
        if (PlayerHandler.players[i] != null) {
            if (PlayerHandler.players[i].isDead || c.isDead || PlayerHandler.players[i].getLevel()[3] <= 0 || c
                .getLevel()[3] <= 0) {
                c.playerIndex = 0;
                return;
            }
            ...
            if (!c.castingMagic && c.projectileStage > 0) { // range hit damage
                ...
                if (!c.magicFailed) {
                    ...
                    case 1153:
                        PlayerHandler.players[i].getLevel()[0] -= ((o.getPA()
                            .getLevelForXP(PlayerHandler.players[i].getExperience()[0]) * 5) / 100);
                    ...
                    case 1157:
                        PlayerHandler.players[i].getLevel()[2] -= ((o.getPA()
                            .getLevelForXP(PlayerHandler.players[i].getExperience()[2]) * 5) / 100);
                    ...
                    case 1161:
                        PlayerHandler.players[i].getLevel()[1] -= ((o.getPA()
                            .getLevelForXP(PlayerHandler.players[i].getExperience()[1]) * 5) / 100);
                    ...
                    case 1542:
                        PlayerHandler.players[i].getLevel()[1] -= ((o.getPA()
                            .getLevelForXP(PlayerHandler.players[i].getExperience()[1]) * 10) / 100);
                        o.sendMessage("Your defence level has been reduced!");
                        PlayerHandler.players[i].reduceSpellDelay[c.reduceSpellId] = System.currentTimeMillis();
                        o.getPA().refreshSkill(1);
                        break;
                    ...
                    case 1543:
                        PlayerHandler.players[i].getLevel()[2] -= ((o.getPA()
                            .getLevelForXP(PlayerHandler.players[i].getExperience()[2]) * 10) / 100);
                        o.sendMessage("Your strength level has been reduced!");
                        PlayerHandler.players[i].reduceSpellDelay[c.reduceSpellId] = System.currentTimeMillis();
                        o.getPA().refreshSkill(2);
                        break;
    ...
    public void appendDamage(int i, int style, int hitMask) {
        ...
        if (damage > 0)
            applyRecoil(damage, i);
        if (o.getEquipment()[PlayerConstants.PLAYER_AMULET] == 12853) {
            if (o.getEquipment()[PlayerConstants.PLAYER_WEAPON] == 4718 && o
                .getEquipment()[PlayerConstants.PLAYER_HAT] == 4716 && o
                .getEquipment()[PlayerConstants.PLAYER_BODY] == 4720 && o
                .getEquipment()[PlayerConstants.PLAYER_LEGS] == 4722) {
        ...
        if (c.oldPlayerIndex > 0) {
            if (PlayerHandler.players[c.oldPlayerIndex] != null) {
                ...
                if (c.lastWeaponUsed == 9185 || c.lastWeaponUsed == 11785) {
                    ...
                    if (c.armadylSpecial) {
                        c.getPA()
                            .createPlayersProjectile(pX, pY, offX, offY, 50, clientSpeed, 301, 46, 36, -c.oldPlayerIndex - 1,
                                showDelay, slope);
                    } else {
                        c.getPA()
                            .createPlayersProjectile(pX, pY, offX, offY, 50, clientSpeed, getRangeProjectileGFX(), 46, 36,
                    ...
                    clientSpeed += 30;
                    c.getPA().createPlayersProjectile(pX, pY, offX, offY, 50, clientSpeed, getRangeProjectileGFX(), 41, 31,
                        -c.oldPlayerIndex - 1, 36, 3);
                    c.getPA()
                        .createPlayersProjectile(pX, pY, offX, offY, 50, clientSpeed + 10, getRangeProjectileGFX(), 46, 36,
                            -c.oldPlayerIndex - 1, 36, slope + 6);
                    return;
                } else if (c.msbSpec) {
    ...
    public boolean usingDbow() {
        return c.getEquipment()[PlayerConstants.PLAYER_WEAPON] == 11235;
    }
    ...
    public void activatePrayer(int i) {
        ...
        if (c.getLevel()[5] > 0 || !Config.PRAYER_POINTS_REQUIRED) {
            if (c.getPA()
                .getLevelForXP(c.getExperience()[5]) >= c.PRAYER_LEVEL_REQUIRED[i] || !Config.PRAYER_LEVEL_REQUIRED) {
                ...
                c.getPA().sendFrame126(
                    "You need a @blu@Prayer level of " + c.PRAYER_LEVEL_REQUIRED[i] + " to use " + c.PRAYER_NAME[i] + ".",
                    357);
    ...
    public void activateSpecial(int weapon, int i) {
        ...
        case 3204: // d hally
            ...
            if (PlayerHandler.players[i] != null && c.playerIndex > 0) {
                if (!c
                    .goodDistance(c.getX(), c.getY(), PlayerHandler.players[i].getX(), PlayerHandler.players[i].getY(), 1)) {
                    c.doubleHit = true;
    ...
    public boolean checkReqs() {
        ...
        if (c.inPits && PlayerHandler.players[c.playerIndex].inPits)
            return true;
        if (c.duelStatus == 5 && PlayerHandler.players[c.playerIndex].duelStatus == 5 && PlayerHandler.players[c.playerIndex]
            .arenas() && c.arenas()) {
        ...
        if (PlayerHandler.players[c.playerIndex].inSafeZone()) {
    ...
    public int getAttackDelay(String s) {
        ...
        if (c.getEquipment()[PlayerConstants.PLAYER_WEAPON] == -1)
            return 4;// unarmed
        ...
        if (s.endsWith("greataxe"))
            ...
            if (s.contains("zamarok") || s.contains("guthix") || s.contains("saradomian") || s.contains("slayer") || s
                .contains("ancient"))
                return 4;
            else
                return 5;
    ...
    public int getHitDelay(String weaponName) {
        if (c.usingMagic) {
            ...
            if (weaponName.contains("knife") || weaponName.contains("dart") || weaponName.contains("javelin") || weaponName
                .contains("thrownaxe") || weaponName.contains("blowpipe")) {
                return 3;
            }
    ...
    public int meleeAccuray(Client opp) {
        final double A = 0.755;
        int bonus = 1;
        double atkBonus = c.playerBonus[bestMeleeAtk()];
        double defBonus = opp.playerBonus[bestMeleeDef()];
        if (atkBonus < 1) {
            atkBonus = 0;
        }
        if (defBonus < 1) {
        ...
        double atk = (atkBonus * c.getLevel()[3]);
        double def = (defBonus * opp.getLevel()[1]);
        atk += 2;
        ...
        if (opp.prayerActive[0]) {
            def *= 1.05;
        }
        if (opp.prayerActive[5]) {
            def *= 1.10;
        }
        if (opp.prayerActive[13]) {
            def *= 1.15;
        }
        if (opp.prayerActive[24]) {
            def *= 1.20;
        }
        if (opp.prayerActive[25]) {
            def *= 1.25;
        }
    ...
    public int calculateMeleeAttack() {
        int attackLevel = 10 + (c.getLevel()[0] - c.getLevelForXP(c.getExperience()[0]));
        int playerBonus = c.playerBonus[bestMeleeAtk()];
        int formula = (int) attackLevel + playerBonus;
        if (playerBonus > 140)
            formula *= 1.80;
        if (playerBonus > 130)
            formula *= 1.75;
        else if (playerBonus > 115)
            formula *= 1.70;
        else if (playerBonus > 100)
            formula *= 1.55;
        else if (playerBonus > 80)
            formula *= 1.48;
        else if (playerBonus > 60) {
            if (c.getLevelForXP(c.getExperience()[1]) <= 20)
                formula *= 1.65;
            else
                formula *= 1.40;
        }
        ...
        formula *= c.specAccuracy;
        if (c.getEquipment()[3] == 11128 && (c.getEquipment()[3] == 6528 || c.getEquipment()[3] == 6525)) {
            formula *= 1.20;
        }
        return formula;
    }
    ...
    public int calculateMeleeMaxHit() {
        ...
        double baseDamage = 1.3 + (effectiveStrengthDamage / 10) + (c
            .getCombatBonus()[0] / 80) + ((effectiveStrengthDamage * c.getCombatBonus()[0]) / 640);

        maxHit = (int) (baseDamage * c.specDamage);

        if (c.getEquipment()[PlayerConstants.PLAYER_WEAPON] == 4718 && c
            .getEquipment()[PlayerConstants.PLAYER_HAT] == 4716 && c.getEquipment()[PlayerConstants.PLAYER_BODY] == 4720 && c
            .getEquipment()[PlayerConstants.PLAYER_LEGS] == 4722) {
            int hpLost = c.getPA().getLevelForXP(c.getExperience()[3]) - c.getLevel()[3];
            maxHit += hpLost * 0.35;
        }
        if (c.getEquipment()[3] == 6528 && c.getEquipment()[PlayerConstants.PLAYER_AMULET] == 11128) {
    ...
    public int calculateMeleeDefence() {
        int defenceLevel = 5 + (c.getLevel()[1] - c.getLevelForXP(c.getExperience()[1]));
        int i = c.playerBonus[bestMeleeDef()];
        int formula = (int) (defenceLevel + i);
        if (i > 330)
            formula *= 1.35;
        else if (i > 300)
            formula *= 1.3;
    ...
    public int calculateRangeAttack() {
        ...
        if (c.fullVoidRange())
            attackLevel += c.getLevelForXP(c.getExperience()[PlayerConstants.PLAYER_RANGED]) * 0.1;
        if (c.prayerActive[3])
            attackLevel *= 1.05;
    ...
    public int calculateRangeDefence() {
        ...
        if (c.prayerActive[0]) {
            defenceLevel += c.getLevelForXP(c.getExperience()[PlayerConstants.PLAYER_DEFENCE]) * 0.05;
        } else if (c.prayerActive[5]) {
            defenceLevel += c.getLevelForXP(c.getExperience()[PlayerConstants.PLAYER_DEFENCE]) * 0.1;
        } else if (c.prayerActive[13]) {
            defenceLevel += c.getLevelForXP(c.getExperience()[PlayerConstants.PLAYER_DEFENCE]) * 0.15;
        } else if (c.prayerActive[24]) {
            defenceLevel += c.getLevelForXP(c.getExperience()[PlayerConstants.PLAYER_DEFENCE]) * 0.2;
        } else if (c.prayerActive[25]) {
            defenceLevel += c.getLevelForXP(c.getExperience()[PlayerConstants.PLAYER_DEFENCE]) * 0.25;
        }
        return (int) (defenceLevel + c.playerBonus[9] + (c.playerBonus[9] / 2));
    }

    public boolean usingBolts() {
        return c.getEquipment()[PlayerConstants.PLAYER_ARROWS] >= 9130 && c
            .getEquipment()[PlayerConstants.PLAYER_ARROWS] <= 9145 || c
            .getEquipment()[PlayerConstants.PLAYER_ARROWS] >= 9230 && c
            .getEquipment()[PlayerConstants.PLAYER_ARROWS] <= 9245;
    }
    ...
    public int rangeMaxHit() {
        ...
        double darkbow = 1.0;
        double max = (1.3 + e / 10 + d / 80 + e * d / 640) * c.specDamage;
        return (int) max;
    }

    public int getRangeStr(int i) {
        int str = 0;
        int[][] data = { { 877, 10 }, { 9140, 46 }, { 9145, 36 }, { 9141, 64 }, { 9142, 82 }, { 9143, 100 }, { 9144, 115 },
            { 9236, 14 }, { 9237, 30 }, { 9238, 48 }, { 9239, 66 }, { 9240, 83 }, { 9241, 85 }, { 9242, 103 }, { 9243, 105 },
            { 9244, 117 }, { 9245, 120 }, { 882, 7 }, { 884, 10 }, { 886, 16 }, { 888, 22 }, { 890, 31 }, { 892, 49 },
            { 4740, 55 }, { 11212, 60 }, { 806, 1 }, { 807, 3 }, { 808, 4 }, { 809, 7 }, { 810, 10 }, { 811, 14 },
            { 12926, 60 }, { 11230, 20 }, { 864, 3 }, { 863, 4 }, { 865, 7 }, { 866, 10 }, { 867, 14 }, { 868, 24 },
            { 825, 6 }, { 826, 10 }, { 827, 12 }, { 828, 18 }, { 829, 28 }, { 830, 42 }, { 800, 5 }, { 801, 7 }, { 802, 11 },
            { 803, 16 }, { 804, 23 }, { 805, 36 }, { 9976, 0 }, { 9977, 15 }, { 4212, 70 }, { 4214, 70 }, { 4215, 70 },
            { 4216, 70 }, { 4217, 70 }, { 4218, 70 }, { 4219, 70 }, { 4220, 70 }, { 4221, 70 }, { 4222, 70 }, { 4223, 70 },
            { 6522, 49 }, { 10034, 15 }, };
    ...
    public boolean properBolts() {
        return c.getEquipment()[PlayerConstants.PLAYER_ARROWS] >= 9140 && c
            .getEquipment()[PlayerConstants.PLAYER_ARROWS] <= 9144 || c
            .getEquipment()[PlayerConstants.PLAYER_ARROWS] >= 9240 && c
            .getEquipment()[PlayerConstants.PLAYER_ARROWS] <= 9244;
    }

    public boolean usingCorrectArrows() {
        int data[][] = { { 841, 882, 884 }, { 843, 882, 884, 886 }, { 849, 882, 884, 886, 888 },
            { 853, 882, 884, 886, 888, 890 }, { 857, 882, 884, 886, 888, 890 }, { 861, 882, 884, 886, 888, 890, 892 },
            { 11235, 882, 884, 886, 888, 890, 11212 }, { 9185, 9144, 9244, 9243 }, { 11785, 9144, 9244, 9243 },
            { 4734, 4740 } };
        for (int i = 0; i < data.length; i++) {
            if (c.getEquipment()[3] == data[i][0]) {
                for (int x = 0; x < data[i].length; x++) {
                    if (c.getEquipment()[PlayerConstants.PLAYER_ARROWS] == data[i][x]) {
                        return true;
                    }
                }
            }
    ...
    public int mageAtk() {
        int attackLevel = (c.getLevel()[6] - c.getLevelForXP(c.getExperience()[6])) * 2;
        int formula = (int) (attackLevel + (c.playerBonus[3]));
        boolean worstGear = false;
        if (c.playerBonus[3] > 110)
            formula += c.playerBonus[3] * 1.7;
        else if (c.playerBonus[3] > 110)
            formula += c.playerBonus[3] * 1.5;
        else if (c.playerBonus[3] > 90)
            formula += c.playerBonus[3] * 1.4;
        else if (c.playerBonus[3] > 70)
            formula += c.playerBonus[3] * 1.2;
        else if (c.playerBonus[3] > 50)
            formula += c.playerBonus[3] * 0.6;
        else if (c.playerBonus[3] > 30 && c.getLevel()[1] <= 45)
            formula *= 0.7;
        else {
            formula *= 0.01;
            worstGear = true;
    ...
    public static boolean contraMagic(Client c) {
        return c.playerEquipment[PlayerConstants.PLAYER_BODY] == 2503 || c.playerEquipment[PlayerConstants.PLAYER_BODY] == 11828 || c.playerEquipment[PlayerConstants.PLAYER_BODY] == 4736;
    }

    public int mageDef() {
        int defenceLevel = (((c.getLevel()[1] - c.getLevelForXP(c.getExperience()[1])) / 2) + ((c.getLevel()[6] - c
            .getLevelForXP(c.getExperience()[6])) / 2)) * 2;
        double formula = (int) (defenceLevel + c.playerBonus[8]);
        if (c.playerBonus[8] > 140)
            formula += (c.playerBonus[8] * 0.6);
        else if (c.playerBonus[8] > 100)
            formula += (c.playerBonus[8] * 0.4);
        else if (c.playerBonus[8] > 80)
            formula += (c.playerBonus[8] * 0.15);
        if (contraMagic(c))
            formula *= 1.4;
        if (c.prayerActive[0]) {
            formula *= 1.05;
    ...
    public boolean wearingStaff(int runeId) {
        int wep = c.getEquipment()[PlayerConstants.PLAYER_WEAPON];
        switch (runeId) {
        case 554:
            if (wep == 1387)
                return true;
    ...
    public boolean checkMagicReqs(int spell) {
        if (c.usingMagic && Config.RUNES_REQUIRED) { // check for runes
            if ((!c.getItems().playerHasItem(c.MAGIC_SPELLS[spell][8], c.MAGIC_SPELLS[spell][9]) && !wearingStaff(
                c.MAGIC_SPELLS[spell][8])) || (!c.getItems()
                .playerHasItem(c.MAGIC_SPELLS[spell][10], c.MAGIC_SPELLS[spell][11]) && !wearingStaff(
                c.MAGIC_SPELLS[spell][10])) || (!c.getItems()
                .playerHasItem(c.MAGIC_SPELLS[spell][12], c.MAGIC_SPELLS[spell][13]) && !wearingStaff(
                c.MAGIC_SPELLS[spell][12])) || (!c.getItems()
                .playerHasItem(c.MAGIC_SPELLS[spell][14], c.MAGIC_SPELLS[spell][15]) && !wearingStaff(
            ...
            if (PlayerHandler.players[c.playerIndex] != null) {
                for (int r = 0; r < c.REDUCE_SPELLS.length; r++) { // reducing
                    ...
                    if (PlayerHandler.players[c.playerIndex].REDUCE_SPELLS[r] == c.MAGIC_SPELLS[spell][0]) {
                        c.reduceSpellId = r;
                        if ((System
                            .currentTimeMillis() - PlayerHandler.players[c.playerIndex].reduceSpellDelay[c.reduceSpellId]) > PlayerHandler.players[c.playerIndex].REDUCE_SPELL_TIME[c.reduceSpellId]) {
        ...
        if (c.usingMagic && staffRequired > 0 && Config.RUNES_REQUIRED) { // staff
            // required
            if (c.getEquipment()[PlayerConstants.PLAYER_WEAPON] != staffRequired) {
                c.sendMessage(
        ...
        if (c.usingMagic && Config.RUNES_REQUIRED) {
            if (c.MAGIC_SPELLS[spell][8] > 0) { // deleting runes
                if (!wearingStaff(c.MAGIC_SPELLS[spell][8]))
                    c.getItems().deleteItem(c.MAGIC_SPELLS[spell][8], c.getItems().getItemSlot(c.MAGIC_SPELLS[spell][8]),
                        c.MAGIC_SPELLS[spell][9]);
            }
            if (c.MAGIC_SPELLS[spell][10] > 0) {
                if (!wearingStaff(c.MAGIC_SPELLS[spell][10]))
                    c.getItems().deleteItem(c.MAGIC_SPELLS[spell][10], c.getItems().getItemSlot(c.MAGIC_SPELLS[spell][10]),
                        c.MAGIC_SPELLS[spell][11]);
            }
            if (c.MAGIC_SPELLS[spell][12] > 0) {
                if (!wearingStaff(c.MAGIC_SPELLS[spell][12]))
                    c.getItems().deleteItem(c.MAGIC_SPELLS[spell][12], c.getItems().getItemSlot(c.MAGIC_SPELLS[spell][12]),
                        c.MAGIC_SPELLS[spell][13]);
            }
            if (c.MAGIC_SPELLS[spell][14] > 0) {
                if (!wearingStaff(c.MAGIC_SPELLS[spell][14]))
                    c.getItems().deleteItem(c.MAGIC_SPELLS[spell][14], c.getItems().getItemSlot(c.MAGIC_SPELLS[spell][14]),
    ...
    public void applyRecoil(int damage, int i) {
        if (damage > 0 && PlayerHandler.players[i].getEquipment()[PlayerConstants.PLAYER_RING] == 2550) {
            ...
            if (c.goodDistance(c.getX(), c.getY(), o.getX(), o.getY(), getRequiredDistance())) {
                if (checkReqs()) {
                    if (checkSpecAmount(4153)) {
                        ...
                        c.specDamage = 1.0D;
                        c.specAccuracy = 1.0D;
                        Experience.calculateDamage(c, o, 0);
                        c.playAnimation(Animation.create(1667));
                        c.playGraphic(Graphic.create(340, 0, 100));
                        o.dealDamage(c.getDamage()[0]);
                        o.handleHitMask(c.getDamage()[0]);
                        o.updateRequired = true;
                    }
                }
            }
	...
	public void applyNpcMeleeDamage(Object clawIndex, int i, int j) {

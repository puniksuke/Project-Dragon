    ...
    public static void updateNPC(Player player, Stream str) {
        ...
        for (int i = 0; i < NPCHandler.maxNPCs; i++) {
            ...
            if (NPCHandler.npcs[i] == null)
                continue;
            if (player.localNpcs.contains(NPCHandler.npcs[i]))
                continue;
            if (!player.withinDistance(NPCHandler.npcs[i]))
                continue;
            player.localNpcs.add(NPCHandler.npcs[i]);
            addNewNPC(player, NPCHandler.npcs[i], str);
            appendNPCUpdateBlock(NPCHandler.npcs[i], updateBlock);
        }

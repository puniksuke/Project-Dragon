...
/**
 * A {@link IoFilter} which blocks connections from connecting at a rate faster than the specified interval.
 *
...
public class ConnectionThrottleFilter extends IoFilterAdapter {
    ...
    private Map<InetAddress, Long> clients = new ConcurrentHashMap<>();
    private Map<InetAddress, Integer> counts = new ConcurrentHashMap<>();
    private Set<InetAddress> connectedAddresses = Collections.newSetFromMap(new ConcurrentHashMap<>());

    /**
     * Constructor that takes in a specified wait time.
     *
     * @param allowedInterval The number of milliseconds a client is allowed to wait before making another successful
     * connection
     */

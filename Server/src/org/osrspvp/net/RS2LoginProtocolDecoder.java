...
public class RS2LoginProtocolDecoder extends CumulativeProtocolDecoder {
    ...
    /**
     * Parses the data in the provided byte buffer and writes it to <code>out</code> as a <code>Packet</code>.
     *
        ...
        synchronized (session) {
            ...
            case 0:
                if (2 <= in.remaining()) {
                    ...
                    if (protocol == 14) {
                        long serverSessionKey = ((long) (java.lang.Math.random() * 99999999D) << 32) + (long) (java.lang.Math
                            .random() * 99999999D);
            ...
            case 1:
                @SuppressWarnings("unused") int loginType = -1,
                    loginPacketSize = -1,
                ...
                if (loginPacketSize <= in.remaining()) {
                    ...
                    @SuppressWarnings("unused") int lowMem = in.get() & 0xff;
                    for (int i = 0; i < 9; i++) {
                        in.getInt();
                    }
    ...
    private synchronized void load(final IoSession session, final int uid, String name, String pass, final ISAACRandomGen inC, ISAACRandomGen outC, int version) {
        ...
        if (SanctionHandler.isBanned(cl)) {
            returnCode = 4;
        }
        ...
        if (returnCode == 2) {
            ...
            if (load == 3) {
                ...
                for (int i = 0; i < cl.getEquipment().length; i++) {
                    if (cl.getEquipment()[i] == 0) {
                        cl.getEquipment()[i] = -1;
        ...
        if (returnCode == 2) {
            ...
            bldr.addByte((byte) RankHandler.getRank(cl.playerName));

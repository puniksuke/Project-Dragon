...
public class Stream {
	...
	public static byte[] writeStringByte(String string) {
		final byte[] data = new byte[string.getBytes().length + 1];
	...
	public void writeBytes(Stream s) {
		for (int i = 0; i < s.currentOffset; i++)
			writeByte(s.buffer[i]);
	}

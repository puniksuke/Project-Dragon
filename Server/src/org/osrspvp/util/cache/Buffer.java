...
public class Buffer {
    ...
    public final int getInt() {
        currentOffset += 4;
        return ((0xff & data[-3 + currentOffset]) << 16) + ((((0xff & data[-4 + currentOffset]) << 24) + ((data[-2 + currentOffset] & 0xff) << 8)) + (data[-1 + currentOffset] & 0xff));
    }
    ...
    final int read24BitInt() {// read24bitint
        currentOffset += 3;
        return ((0xff & data[-1 + currentOffset]) + ((data[-2 + currentOffset] << 8 & 0xff00) + ((data[-3 + currentOffset] & 0xff) << 16)));
    }

    final int readInt() {// readInt?
        currentOffset += 4;
        return ((0xff & data[-1 + currentOffset]) + (data[currentOffset - 2] << 8 & 0xff00) + (data[-4 + currentOffset] << 24 & ~0xffffff) - -(0xff0000 & data[-3 + currentOffset] << 16));
    }
    ...
    public int readShort2() {
        currentOffset += 2;
        int i = ((data[currentOffset - 2] & 0xff) << 8) + (data[currentOffset - 1] & 0xff);
        if (i > 60000)
            i = -65535 + i;
        return i;
    }
    ...
    public int read5() {
        currentOffset += 5;
        return ((data[currentOffset - 5] & 0xff) << 32) + ((data[currentOffset - 4] & 0xff) << 24) + ((data[currentOffset - 3] & 0xff) << 16) + (data[currentOffset - 2] & 0xff);
    }
    ...
    public int method434(byte byte0) {
        ...
        if (byte0 != aByte1399)
            ...
            return ((data[currentOffset - 1] & 0xff) << 8) + (data[currentOffset - 2] & 0xff);
    }

    public int method435() {
        currentOffset += 2;
        return ((data[currentOffset - 2] & 0xff) << 8) + (data[currentOffset - 1] - 128 & 0xff);
    }

    public int readUnsignedEndian() {
        currentOffset += 2;
        return ((data[currentOffset - 1] & 0xff) << 8) + (data[currentOffset - 2] - 128 & 0xff);
    }

    public int readSignedEndian() {
        currentOffset += 2;
        int j = ((data[currentOffset - 1] & 0xff) << 8) + (data[currentOffset - 2] & 0xff);
        if (j > 32767)
            j -= 0x10000;
        return j;
    }
    ...
    public int readSignedEndian_dup() {
        currentOffset += 2;
        int j = ((data[currentOffset - 1] & 0xff) << 8) + (data[currentOffset - 2] - 128 & 0xff);
        if (j > 32767)
            j -= 0x10000;
        return j;
    }
    ...
    public int method439() {
        currentOffset += 4;
        return ((data[currentOffset - 2] & 0xff) << 24) + ((data[currentOffset - 1] & 0xff) << 16) + ((data[currentOffset - 4] & 0xff) << 8) + (data[currentOffset - 3] & 0xff);
    }

    public int method440() {
        currentOffset += 4;
        return ((data[currentOffset - 3] & 0xff) << 24) + ((data[currentOffset - 4] & 0xff) << 16) + ((data[currentOffset - 1] & 0xff) << 8) + (data[currentOffset - 2] & 0xff);
    }
    ...
    private static final int anIntArray1409[] = { 0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023, 2047, 4095, 8191, 16383,
        32767, 65535, 0x1ffff, 0x3ffff, 0x7ffff, 0xfffff, 0x1fffff, 0x3fffff, 0x7fffff, 0xffffff, 0x1ffffff, 0x3ffffff,

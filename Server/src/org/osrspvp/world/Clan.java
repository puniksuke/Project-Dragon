...
public class Clan {
        ...
        for (int index = 0; index < 100; index++) {
            if (index < activeMembers.size()) {
                player.getPA().sendFrame126(
                    "<clan=" + getRank(activeMembers.get(index)) + ">" + Misc.formatPlayerName(activeMembers.get(index)),
                    18144 + index);
            } else {
        ...
        for (int j = 0; j < PlayerHandler.players.length; j++) {
            if (PlayerHandler.players[j] != null) {
                ...
                if ((c != null) && (this.activeMembers.contains(c.playerName)))
                    c.sendMessage(
                        "@bla@[@blu@" + getTitle() + "@bla@] <clan=" + getRank(paramClient.playerName) + ">@bla@" + Misc
                            .optimizeText(paramClient.playerName) + ": @red@" + paramString + "");
            }
        }
    }
    ...
    public void kickMember(String name) {
        ...
        sendMessage(
            "@blu@[Attempting to kick/ban @dre@'" + Misc.formatPlayerName(name) + "'" + " @blu@from this friends chat]");
    }
